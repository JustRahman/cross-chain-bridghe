...
class ConnextBridge(BaseBridge):
    """
    Connext integration.

    Connext is a modular protocol for securely passing funds and data between chains.
    Uses nxtp (nomad cross-chain transfer protocol) for fast, trust-minimized bridging.
    ...
    def __init__(self):
        super().__init__()
        self.name = "Connext"
        self.protocol = "connext"
        self.api_url = "https://sdk-server.mainnet.connext.ninja"
        self.supported_chains = [1, 10, 42161, 137, 8453, 56, 100]
    ...
    async def get_quote(self, route_params: RouteParams) -> Optional[BridgeQuote]:
        try:
            ...
            log.error(f"Error getting Connext quote: {e}")
            return None
    ...
    async def _generate_estimated_quote(self, route_params: RouteParams) -> BridgeQuote:
        ...
        # Connext fees: router fee (0.05%) + relayer fee
        router_fee = Decimal(str(round(amount_usd * 0.0005, 2)))
        relayer_fee = Decimal("0.50")  # Fixed relayer fee

        bridge_fee_usd = router_fee + relayer_fee
        gas_cost_source_usd = Decimal(str(self._estimate_gas_cost(source_chain_id)))
        gas_cost_dest_usd = Decimal("0.10")  # Relayer covers most destination gas
        ...
        return BridgeQuote(
            ...
            steps=[
                ...
                {"action": "bridge", "description": f"Bridge via router network to {route_params.destination_chain}"}
            ],
            requires_approval=True,
            minimum_amount="1000000",  # $1
            maximum_amount="1000000000000",  # $1M
            quote_id=f"connext_{int(datetime.utcnow().timestamp())}",
            metadata={"router_liquidity": "true"}
        )

    def _estimate_gas_cost(self, chain_id: int) -> float:
        gas_costs = {1: 7.00, 10: 0.14, 42161: 0.30, 137: 0.07, 8453: 0.14, 56: 0.20, 100: 0.02}
