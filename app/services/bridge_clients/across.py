...
class AcrossClient(BridgeClient):
    """Client for Across Protocol bridge"""
    ...
    async def get_quote(
        ...
        """Get quote from Across Protocol (mock implementation)"""

        if source_chain not in self.supported_chains or destination_chain not in self.supported_chains:
            log.debug(f"Across doesn't support {source_chain} -> {destination_chain}")
            return None
        ...
        try:
            ...
            return {
                ...
                "estimated_time_minutes": 3,  # 3 minutes
